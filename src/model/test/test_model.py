    ...
    def test_getitem(self):
        ...
        for i in v.getints(-2**99, 2**99, 10):
            for j in xrange(gridsize):
                ...
                filled_grid[j, 1, 0] = f'{str(i)}+{str(j)}'
                filled_grid[j, 2, 0] = f'{str(i)}*{str(j)}'                
            ...
            for j, funcname in enumerate(['int', 'gmpy.mpz', 'gmpy.mpq']):
                ...
                filled_grid[1, 0, 0]
                filled_grid[j, 3, 0] = f'{funcname} ({str(i)})'

                res = eval(f"{funcname}(i)")
                assert filled_grid[j, 3, 0] == eval(f"{funcname}(i)")
        #Test X, Y, Z
        for i in xrange(10):
